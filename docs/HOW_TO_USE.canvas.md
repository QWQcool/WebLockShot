...
| **🔥 ComfyUI 私有算力** | **真实本地算力出片**（Wan 2.2 TI2V-5B，产物是 ComfyUI `/view` 的 http 直链） |
...
> ComfyUI 产物依赖本机服务常驻；服务停了产物直链就播不了（这是如实的能力边界，不做本地转存伪装）。
...
2. **本地 ComfyUI 只在 RTX 3080 10GB 上验证过 `wan2.2-ti2v-5b` 预设**：5 秒 704×1280 单条约
   ...
   **接口预留、本机未验证**；设置面板「测试连接」会如实报出缺哪些节点与权重。
   ComfyUI 产物是 `/view` http 直链，**服务停了就播不了**。
...
9. **出片完成后约 400ms 内刷新**：画布文档的防抖落盘尚未完成 → 节点 `meta.artifacts` 指针丢失 →
