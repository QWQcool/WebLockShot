...
- [5. 出片：从脚本到产物卡（含运行历史 🕘）](#5-出片从脚本到产物卡)
...
引擎在「⚙️ API 设置 → 2. 视频生成引擎」统一配置，画布内**隐式**读它，并在 generate 节点上如实显示：

| 设置里选的引擎 | 画布实际行为 |
|---|---|
| **Mock 实验画布** | 离线演示引擎：确定性合成、0 成本、不占显卡 |
| **🔥 ComfyUI 私有算力** | **真实本地算力出片**（Wan 2.2 TI2V-5B）。产物出片后**自动转存本地 IndexedDB**（`idbref://`），离线可播、可打包 |
| 可灵 / 即梦 / Runway / Luma | 画布**未接线** → 回落 Mock，并在节点内如实列出这四个引擎「暂不可用」，不假装可用 |

选 ComfyUI 时另有**质量档**（同一设置面板，10GB 显存档位）：

| 档位 | 分辨率 × 帧数 | steps | 单镜实测/估算 |
|---|---|---|---|
| 快速 | 480×832 × 49（≈2.0s） | 8 | 约 1 分钟（含冷启） |
| 标准 | 704×1280 × 97（≈4.0s） | 16 | 约 4 分钟（外推） |
| 高（原生） | 704×1280 × 121（≈5.0s） | 20 | **实测 10 分 55 秒** |

> 实测基准与节点参数核对记录见 [comfyui-local.md](./comfyui-local.md)（含 6 镜≈1 小时的换算）。
> 点「开始出片」前的确认弹层会先写出**预计耗时**与引擎名，不会让人干等黑屏。

### 运行历史（工具条「🕘 运行历史」）

![运行历史：状态 / 耗时 / 费用（灵感币）/ 是否退款 / 失败原因](./screenshots/19c_canvas_run_history.png)

每次节点执行（逐镜出片 / 重试 / 重生成）都会落一条**可审计记录**，由执行器**单一收口**写入，
滚动保留最近 200 条。面板里能看到：

- **状态**（成功 / 失败 / 进行中）与**耗时**；
- **费用**：该次**实际消耗**的灵感币（0 币显示「0 灵感币（未扣费）」），以及**是否发生退款**——
  失败自动退款会显示「↩ 已退款」；**0 币无可退时不显示该徽章**（避免把免费失败误读成退过款）；
- **失败原因**：点开某条记录，展开区里给出原因（如「上游 4xx」）与「无款项可退 / 未退款（正常核销）」
  的区分；
- 展开区还给出关联节点、分镜、供应商、尝试次数与**输出引用**——演示产物是 `blob:`（刷新即失效），
  面板**优先展示节点内持久化的 `idbref://`**，取不到就如实显示「产物引用已失效」，不伪造；
- **演示引擎（Mock）**产生的记录标注「演示 · 非真实生成」；
- 「🧹 清空」一键清空；记录存 IndexedDB，**刷新后仍在**（顶部汇总给出总次数 / 成功 / 失败 / 净消耗 / 已退回）。
...
1. **可灵 / 即梦 / Runway / Luma 未接入画布出片**（可灵 / 即梦请走带货工作台）；画布出片走
   **Mock 演示引擎**或**本地 ComfyUI 真实算力**（二选一，见上文「出片引擎」）。
2. **本地 ComfyUI 只在 RTX 3080 10GB 上验证过 `wan2.2-ti2v-5b` 预设**：5 秒 704×1280 单条约
   **11 分钟**，且显存打满、依赖 offload（详见 [comfyui-local.md](./comfyui-local.md)）。
   预设 `wan2.1-i2v`（需 Kijai 插件）与 `minimax-h3`（需 32B 文本编码器，10GB 不可行）为
   **接口预留、本机未验证**；设置面板「测试连接」会如实报出缺哪些节点与权重。
3. **产物持久化**：出片后会把上游直链转存本地 IndexedDB（`idbref://`），离线可播可打包；
   **转存失败**（上游不可达 / 跨域受限 / 超过 256MB / 浏览器配额不足）时**不判任务失败**，
   保留直链并在产物卡如实标注「⚠️ 未转存本地 · 依赖上游服务在线」——不假装已持久化。
4. **ComfyUI 离线**时局部重绘降级演示模式并标注「非真实生成」。
5. **记忆纯前端模式仅存本机**；接入 `WLS_STORAGE=sqlite` 伴生服务后升级为服务端记忆。
6. **画布文档契约上限 200 节点 / 400 边**：超限后不再落盘（T3 实测，UI 暂无提示，见主手册遗留台账）。
7. **tldraw 免费版带水印**，生产环境未配置 license key 时渲染约 5 秒后停止；本项目不做任何绕过。
8. **i18n 为「关键文案」覆盖**，节点内部业务控件等仍为中文。
9. **MCP / 连接器目前只有 API 与有限 UI**（MCP 徽章 + 连接器面板），更深的双向编排需要按文档直调接口。
10. **出片完成后约 400ms 内刷新**：画布文档的防抖落盘尚未完成 → 节点 `meta.artifacts` 指针丢失 →
    运行历史取不到 `idbref://` 引用（**资产本身仍在 IndexedDB**，重新出片即可恢复）。窗口很窄，
    本轮如实记录、不做补偿。
