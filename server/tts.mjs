 ...
 */
export async function synthesizeTtsToFile({ text, voice = TTS_DEFAULT_VOICE, rate, ttsDir }) {
 ...
 */
export function createTtsHandler({ enabled, ttsDir, logger, synth = synthesizeTtsToFile }) {
  return async function handleTts(req, res, urlPath) {
    ...
    let body
    try {
      const buf = await readJsonBody(req, TTS_TEXT_MAX * 8 + 1024)
