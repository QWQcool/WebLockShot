...
| 🧾 | **诚实标注 + 可验证工程** | 无 Key 标注「演示编排 · 非真实 LLM」、未实现节点标「未实现」、空态不摆样例数据、许可限制如实告知；**17 步实机 E2E + 子路径部署回归 + 生产模式回归 + 449 项单测 + a11y 双引擎扫描**，全部可本地复现并进 CI |
...
- **🕘 运行历史（P1）**：每次节点执行落一条可审计记录（**执行器单一收口**，滚动保留最近 200 条），面板显示
  **状态 / 耗时 / 费用（灵感币）/ 是否退款 / 失败原因**，可展开看关联节点、分镜、供应商、尝试次数与输出引用
  （优先展示节点内持久化的 `idbref://`）；演示引擎记录标注「演示 · 非真实生成」，支持一键清空

![运行历史：状态 / 耗时 / 费用（灵感币）/ 是否退款 / 失败原因（图中展开的失败条为截图脚本写入的演示记录）](./docs/screenshots/19c_canvas_run_history.png)

**诚实边界（逐条如实标注，不夸大）**：
- **可灵 / 即梦 / Runway / Luma 在画布模式未接线**（可灵 / 即梦请走带货工作台）；画布出片走
  **Mock 演示引擎**或**本地 ComfyUI 真实算力**（在「⚙️ API 设置 → 2. 视频生成引擎」里二选一，画布内隐式生效）
- **本地 ComfyUI 仅 `wan2.2-ti2v-5b` 预设经本机实跑验证**（RTX 3080 10GB：5 秒 704×1280 单条约 **11 分钟**，
  快速档 480×832 约 30 秒；显存打满、依赖 offload）。`wan2.1-i2v`（需 Kijai 插件）与 `minimax-h3`
  （需 32B 文本编码器，10GB 不可行）为**接口预留、本机未验证**；设置面板会如实报出缺哪些节点与权重。
  另外**浏览器不能直连 `127.0.0.1:8188`**（ComfyUI 新版校验回环 Host/Origin，POST 必 403），
  必须经同源反代 `/api/comfyui` —— 详见 [docs/comfyui-local.md](./docs/comfyui-local.md)
- **本地算力出片后会把产物转存本地 IndexedDB**（`idbref://`，离线可播可打包）；转存失败时保留直链并
  在产物卡标注「⚠️ 未转存本地」，不判任务失败、也不假装已持久化
...
├── scripts/
│   ├── e2e-canvas.mjs                # T1 画布 E2E 套件（17 步）
...
不使用设计稿或手绘 mock。两处「有数据」截图的图注已如实写明来源：**记忆图谱**（脚本写入本地 IndexedDB 的
60 条回流记录）与**运行历史**（真实跑完 6 镜出片 + 1 条脚本写入的失败/退款演示记录，用于体现「费用 · 已退款 ·
失败原因」列）。
...
| 切片 | 结论 |
|---|---|
| T1 E2E 固化 | `npm run e2e` 17 步全过（真实鼠标路径 + 隔离 storage；含 T9「节点管线实跑」、T10「悬浮提示对比度守卫」与 P1「运行历史」两步）；Playwright 缺失时优雅跳过 exit 0 |
...
| T11 许可文案过期 + 闸门状态单一来源 | 配置 `TLDRAW_LICENSE_KEY` 后，顶部说明条仍固定展示「水印与生产环境约 5 秒停渲染为 tldraw 免费版限制」（**过期文案**）。根因是它位于 `<Tldraw>` 子树内且文案写死——实测闸门触发后该说明条**自身会被卸载**，写在里面根本告知不到。修复：说明条只留静态产品标语，许可条件统一交给闸门**之外**的 `TldrawLicenseNotice`；抽出 `useTldrawGate()` 作为闸门状态**单一来源**（消掉两处各自判断的口径漂移）。守卫：`i18n.test.ts` 新增「静态文案禁止写死运行期许可条件」单测 + `e2e:prod` 双向断言（未授权必须讲清、已授权不得出现）+ **key 到期日解析与剩余 ≤30 天告警**（当前 trial 到期 2026-12-20） |
| T10 悬浮提示「空白」修复 | 用户实机反馈「鼠标移过去提示描述为空」。根因：浅色皮肤把 tldraw 的 `--tl-color-tooltip` 覆盖成 `#ffffff`，却没有同时改 `--tl-color-text-shadow`（`.tl-theme__light` 下仍是白色）→ tooltip **白底白字**，看起来就是个空框（探针实测 `1.00:1`）。修复：删掉该覆盖，回到 tldraw 自带配对（light = 深底白字）。**新增硬守卫**：E2E 第⑩步直接量测 tooltip 计算色并算 WCAG 对比度，要求 ≥ 4.5:1（负向验收：加回错误覆盖 → 立即报 `对比度不足（1.00:1 …）`） |
...
9. **tldraw 许可闸门（已配置 key，当前不受影响）**：https 非回环地址 + **无有效 key** 时，tldraw 按许可
   条款在约 5 秒后停止渲染编辑器（`<Tldraw>` 子树被替换为空 div）。现已在仓库 Secrets 配置
   `TLDRAW_LICENSE_KEY` 并由 CI 构建期注入 → **线上产物已烘焙 key，当前不会再停渲染**；
   未授权时由闸门**之外**的**如实提示条**说明原因与官方解决路径（放在 `<Tldraw>` 内的说明条会随编辑器
   一起被卸载，告知不到——2026-09-11 实测）。
   ⚠️ **当前 key 为免费 trial，2026-12-20 到期；官方文档明确「评估（试用）许可没有宽限期，到期立即失效」**
   —— 到期当天**所有访客**的画布会一起消失。`e2e:prod` 已加**到期日解析 + 剩余 ≤30 天告警**（按既有口径
   只告警不阻断部署）。本项目**不提供绕过校验 / 去水印手段**（用户已拍板）。详见 [NOTICE](./NOTICE)。
...
13. **出片完成后约 400ms 内刷新页面** → 画布文档的**防抖落盘尚未完成** → 节点 `meta.artifacts` 指针丢失 →
    运行历史（与节点）取不到 `idbref://` 引用。**窗口很窄**（画布文档 400ms 防抖），且**资产本身仍在
    IndexedDB**（`canvas-asset-<shotId>` key），**重新出片即可恢复**；本轮如实记录，不做补偿。
