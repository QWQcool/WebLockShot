...
| **D3 出片衔接（B+D+C 口，图7 语义；D 路径=自由镜数）** | 导出物 = 机位帧序列（渲染图 + 相机参数 + 轨迹结构化数据 + 运镜文字）；**B**=单机位首尾帧+分段描述直送 generate（图生视频，演示引擎 + 诚实标注）；**D**=多机位帧整组送 storyboard，走 **canvas 原生自由镜数分镜**（镜数=机位数，1~12 不伪造不截断；canvas 侧新增可变镜数分镜契约与「3D 台自由分镜」来源模式，与既有 script→6 镜模式并存；**sell 的 6 镜管线零改动**——其硬约束遍布 `ai/schema.ts`/`ai/generate.ts`/`persist.ts`/`types.ts`，属红线不可放宽）；**C** 升级口=轨迹结构化数据落契约层（不丢信息，未来 Provider 放开运动控制 API 时升级） | ① 3D 台导出 → generate 收到首尾帧+运镜文字，演示引擎可出片 + 诚实标注；② 多机位帧整组送 storyboard → 自由镜数分镜落位正确（镜数=机位数）+ 每镜帧图/相机参数/运镜文字齐备；③ 轨迹结构化数据契约 zod+单测（C 口）；④ **不触碰 sell 6 镜契约**（`src/ai/` 与 `src/persist.ts` diff 为零核对）；⑤ 不引入 sell 运镜词典/深度图；⑥ 零回归 + 实机冒烟 |
...
| **D5 画布 UX 对齐（清单二 ①②⑤ + 遗留小修）** | ① 生成引擎画布内**隐式**（引擎配置藏进设置，generate 节点不暴露引擎下拉）；② 画布顶栏**移除钱包余额**，改为出片前「将消耗 N 灵感币」确认弹层；⑤ **图1 风格开场层**（五类场景 tab + 大输入卡 + 连接器条，首次进入画布叠加，之后折叠为现有对话栏）；遗留小修：机位 FOV 表单旁注「编辑后需飞行生效」、记忆图谱小窗口气泡与脚注避让、Skill 市场「发布」双入口统一、B6 编排 system prompt 提取共享常量 | ① 引擎隐式后出片链路与钱包账目零变化（sell 工作台保持显式下拉，零回归）；② 无余额显示但出片前确认弹层金额准确 + 取消不扣费；③ 开场层与图1 形态对齐且「进入画布」后不阻塞画布操作、二次进入不重复弹；④ 四项遗留小修逐项验证 + 台账勾销 |
| **D6 图8 六类创作场景卡片** | 复刻 Miora 图8：六类创作场景卡片（品牌设计/电商物料/影视文娱/游戏内容/产品 UI-UX/宣传物料）作为画布入口层，点击预填对话栏场景与建议拓扑（复用 B6 编排路由） | ① 六卡片形态与图8 对齐（图标/标题/描述/配图）；② 点击卡片 → 对话栏预填 + 可一键编排；③ 与 D5 开场层/模板 chips 关系理顺（不重复入口打架）；④ 实机冒烟 + 零回归 |
...
| **收官（D5~D8 后）** | README v2.4 能力清单与诚实边界回填（含 server API 直写无 UI 开关约束、单图直出仅演示引擎、tldraw 水印状态）；**HOW_TO_USE「一主两分」重构**（画布优先主手册 `docs/HOW_TO_USE.md` + 带货分册 + 短剧分册标注 legacy 边缘化；README hero 改画布优先）；遗留台账清零 | ① README 与实现逐项对账无夸大；② 三份文档可独立走通对应模式；③ 遗留台账全部勾销或明确标注「不修 + 原因」 |

> **tldraw License 路线（v1.7 记录，待用户最终拍板）**：① 开发/自用阶段接受水印（localhost 属开发环境可正常运行，生产无 key 会 5 秒后停渲染）；② 需无水印试用 → 申请官方 **Trial key**（免费 100 天、无水印、每公司限一次，走官方渠道合法获取），代码侧提供 `licenseKey` 注入通道（env `VITE_TLDRAW_LICENSE_KEY` 或设置项）；③ 商业化前联系 tldraw 销售购买 Commercial license（价格未公开）；④ **不做**任何绕过/去除水印或校验的技术手段（违反许可条款，且 tldraw LICENSE 明载技术措施条款）；⑤ 备选退路（若许可成本不可接受）：迁移至宽松许可引擎（Excalidraw MIT / React Flow MIT / 自研最小画布），但需重写 `src/canvas` 与 `src/ui/canvas`，成本高，仅在商业前景明确后评估。
