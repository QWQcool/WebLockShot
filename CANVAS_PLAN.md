...
| **Spike 素体模型选型（前置，不与切片并列提交）** | 候选 three.js Xbot/Ybot / quaternius / Mixamo：许可核查（商用）+ 观感对比 + 骨骼/姿势能力验证 + 包体评估 | 产出选型结论（模型文件 + 许可 + 姿势方案），写入 D1 方案，用户确认后开 D1 |
...
| **D1 3D 运镜台基座（图7 1:1）** | three.js + @react-three/fiber(v9, React19) + drei 独立 chunk 懒加载；stage3d 双击进全屏页；布局四件套 1:1（左栏/右属性面板/底部工具条/导演视角+重置视角）；写实素体（Spike 选型产物）；位置/旋转/缩放（带锁）/8 色板；无限网格地面；全景图导入；全程本地渲染 0 灵感币 | ① 全屏页布局与图7 四件套对齐；② 素体拖拽摆位/XYZ/色板真实鼠标可用；③ 摆台状态持久化（meta 契约 zod+单测）；④ 3D chunk 不进主包（构建核对）；⑤ 无 WebGL 灰态诚实降级；⑥ 实机冒烟 |
