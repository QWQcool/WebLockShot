...
| **D8（可选）MCP 真实接入 1 个标杆连接器** | 按用户「可选依赖」思路：**默认零依赖不变**，伴生服务动态检测 `@modelcontextprotocol/sdk` 是否已安装（try/catch 动态 import）→ healthz 能力位 `connectors: 'ready' \| 'interface'`；未安装时 UI 显示安装指引（复制 `npm i @modelcontextprotocol/sdk` 命令 + 重启提示），安装后走真实 MCP；首个标杆目标（GitHub 或 Resend，PAT/API key 授权） | ① 未安装 = 现状行为（接口 + 诚实标注）零变化；② 安装后能力位切 'ready' + 真实调用打通 1 个连接器（授权→取需求/回成品闭环）；③ 不装依赖时服务器零报错（动态 import 失败优雅降级）；④ 文档交付 + 零回归 |
| **收官（D5~D8 后）** | README v2.4 能力清单与诚实边界回填（含 server API 直写无 UI 开关约束、单图直出仅演示引擎、tldraw 水印状态）；**HOW_TO_USE 三版重构**（画布优先主手册 + 带货分册 + 短剧分册边缘化）；遗留台账清零 | ① README 与实现逐项对账无夸大；② 三版文档可独立走通对应模式；③ 遗留台账全部勾销或明确标注「不修 + 原因」 |
